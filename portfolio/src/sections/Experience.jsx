...
import { MapPin, CalendarRange, Link2, X, MousePointerClick, ArrowRight, ArrowLeft, ImagePlus } from 'lucide-react'
import './Experience.css'
import './ExperienceRails.css'
...
/* ordre chronologique : gauche = les 4 premiers, droite = les 3 derniers */
const TIMELINE = Object.entries(ENTRIES).sort((a, b) => a[1].order - b[1].order)
const LEFT = TIMELINE.slice(0, 4)
const RIGHT = TIMELINE.slice(4)

/* ── Wrapper ─ */
function Hotspot({ id, type, label, cue, halo, tagY, sel, setSel, focus, children }) {
  const active = sel === id
  const w = label.length * 7.4 + 22
  return (
    <g className={`hot hot--${type} ${active ? 'is-active' : ''} ${focus === id ? 'is-focus' : ''}`}
...
function RailCard({ id, entry, side, sel, focus, setFocus, onPick, broken, onBroken }) {
  const active = sel === id
  const Arrow = side === 'right' ? ArrowLeft : ArrowRight
  return (
    <li className="rail__item">
      <button
        type="button"
        className={`rail-card rail-card--${entry.type} ${active ? 'is-active' : ''} ${focus === id ? 'is-focus' : ''}`}
        aria-pressed={active}
      ...
      >
        <span className="rail-card__thumb">
          {broken[id] ? (
            <span className="rail-card__ph"><ImagePlus size={18} strokeWidth={2.2} /></span>
          ) : (
            <img src={`/parcours/${id}.png`} alt="" loading="lazy" onError={() => onBroken(id)} />
          )}
        </span>
        <span className="rail-card__body">
          <span className="rail-card__period">{entry.period}</span>
          <span className="rail-card__title">{entry.title}</span>
          <span className="rail-card__role">{entry.role}</span>
        </span>
        <span className="rail-card__go" aria-hidden="true"><Arrow size={15} strokeWidth={2.8} /></span>
      </button>
    </li>
  )
}

/* ── Une colonne de la frise ─ */
function Rail({ items, side, cap, ...rest }) {
  return (
    <div className={`rail-wrap rail-wrap--${side}`}>
      {cap && <p className="rail__cap mono">{cap}</p>}
      <ol className={`rail rail--${side}`}>
        {items.map(([id, entry]) => (
          <RailCard key={id} id={id} entry={entry} side={side} {...rest} />
        ))}
      </ol>
    </div>
  )
...
export default function Experience() {
  ...
  const onBroken = (id) => setBroken((b) => (b[id] ? b : { ...b, [id]: true }))

  const onPick = (id) => {
    setSel(id)
    if (roomRef.current && window.matchMedia('(max-width: 1180px)').matches) {
      roomRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' })
    }
  }

  const railProps = { sel, focus, setFocus, onPick, broken, onBroken }
    ...
    <section id="experience" className="experience">
      <div className="experience__inner">
        <header className="experience__head">
          ...
          <div className="experience__subrow">
            <p className="experience__hint">
              ...
              Clique sur un objet marqué d’un point coloré — ou sur une étape de la frise.
            </p>
        ...
        <div className="room-stage">
          <Rail items={LEFT} side="left" cap="2019 ↓" {...railProps} />

          <div className="room-frame" ref={roomRef}>
            <div className="room-clip">
              <svg viewBox="0 0 1000 640" className="room" role="img"
                ...
                <Hotspot id="lycee" type="formation" label="Lycée Lacordaire"
                  cue={{ x: 200, y: 104 }} halo={{ cx: 200, cy: 168, rx: 82, ry: 76 }} tagY={84} sel={sel} setSel={setSel} focus={focus}>
                ...
                <Hotspot id="capgemini" type="experience" label="Capgemini"
                  cue={{ x: 880, y: 392 }} halo={{ cx: 880, cy: 500, rx: 72, ry: 128 }} tagY={372} sel={sel} setSel={setSel} focus={focus}>
                ...
                <Hotspot id="brother" type="experience" label="Brother France"
                  cue={{ x: 710, y: 404 }} halo={{ cx: 710, cy: 510, rx: 88, ry: 110 }} tagY={384} sel={sel} setSel={setSel} focus={focus}>
                ...
                <Hotspot id="epitech" type="formation" label="EPITECH"
                  cue={{ x: 540, y: 404 }} halo={{ cx: 540, cy: 500, rx: 150, ry: 116 }} tagY={384} sel={sel} setSel={setSel} focus={focus}>
                ...
                <Hotspot id="college" type="experience" label="Collège Lacordaire"
                  cue={{ x: 235, y: 410 }} halo={{ cx: 235, cy: 500, rx: 110, ry: 110 }} tagY={390} sel={sel} setSel={setSel} focus={focus}>
                ...
                <Hotspot id="bde" type="asso" label="BDE Marsatek"
                  cue={{ x: 370, y: 470 }} halo={{ cx: 370, cy: 560, rx: 74, ry: 96 }} tagY={450} sel={sel} setSel={setSel} focus={focus}>
                ...
                <Hotspot id="myongji" type="formation" label="Myongji University"
                  cue={{ x: 85, y: 470 }} halo={{ cx: 85, cy: 540, rx: 78, ry: 96 }} tagY={450} sel={sel} setSel={setSel} focus={focus}>
          ...
          <Rail items={RIGHT} side="right" cap="↓ aujourd’hui" {...railProps} />

          {/* frise horizontale — mobile / tablette uniquement */}
          <Rail items={TIMELINE} side="all" {...railProps} />
        </div>
      </div>
    </section>
  )
